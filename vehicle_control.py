...
"""
This class encapsulates the vheicle and some commonly used controls via the DroneAPI
...
class VehicleControl(object):
    def __init__(self):
        ...
        # search variables
        self.search_state = 0                   # 0 = not searching, 1 = moving horizontally towards target, 2 = descending
        self.search_target_pos = None           # position (as an offset from home) of the target (so far) during search
        self.search_target_north_angle = None   # earth-frame angle (in radians) from vehicle to target in northerly direction
        self.search_target_east_angle = None    # earth-frame angle (in radians) from vehicle to target in easterly direction
        self.search_target_distance = None      # distance (in meters) from vehicle to target

        # vehicle mission
        self.mission_cmds = None
        self.mission_distance_max = 0           # max distance from vehicle to target (we ignore targets further than this distance).  "0" means no limit
        ...
        # target direction and position estimate from latest image analysis
        self.target_found = False
        self.target_pitch = None                # body-frame pitch angle (in radians) from vehicle to target
        self.target_roll = None                 # body-frame roll angle (in radians) from vehicle to target
        self.target_distance = None             # distance (in meters) from vehicle to target
        self.target_pos = None                  # last estimated position as an offset from home
        self.target_last_spotted_time = 0       # time the target was last seen (used for timeout)
        ...
        # roll velocity pid controller.  maximum effect is 10 degree lean
        vel_p = sc_config.config.get_float('general','VEL_XY_P',1.0)
        vel_i = sc_config.config.get_float('general','VEL_XY_I',0.0)
        vel_d = sc_config.config.get_float('general','VEL_XY_D',0.0)
        vel_imax = sc_config.config.get_float('general','VEL_XY_IMAX',10.0)
        self.roll_pid = pid.pid(vel_p, vel_i, vel_d, math.radians(vel_imax))
        self.pitch_pid = pid.pid(vel_p, vel_i, vel_d, math.radians(vel_imax))
